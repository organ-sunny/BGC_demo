<template>
    <div>
        <!-- 模块 -->
        <div :class="actionId === module.id ? 'action' : ''" @click="moduleClick()" style="display: flex;padding: 10px 20px;" class="select">
            <div>
        ...
        <div v-if="isOpen" style="margin-left: 20px;">
            <div :class="!variableUtil.isEmpty(apiActionId) && apiActionId === item.id ? 'action' : ''" @click="selectApiKey = key;apiClick(item)" v-for="(item, key) in api.data" :key="key" style="padding: 10px 20px;" class="select">
